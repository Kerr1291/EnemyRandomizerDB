...
namespace EnemyRandomizerMod
{
    ...
    public partial class EnemyRandomizerDatabase
    {
        ...
        void CreatePrefabObject(string name, GameObject go, SceneObject s)
        {
            if (s.Loaded)
                return;

            Dev.Log("Creating prefab object ");// from path " + go.GetSceneHierarchyPath() + " in scene " + s.Scene.name);
            //Dev.Log("Creating prefab object from path " + go.GetSceneHierarchyPath() + " in scene " + s.Scene.name);
            s.Loaded = true;

            go.SetActive(false);
            go.name = ToDatabaseKey(s.Name);
            ...
            {
                prefabName = ToDatabaseKey(name),
                prefab = go,//GameObject.Instantiate(go),
                source = s
            };
            ...
            if (p.prefab == null)
            {
                Dev.Log("ERROR: PREFAB OBJECT CREATED WITH NULL PREFAB");
            }
            else
            {
            ...
            if (enemyNames.Contains(p.prefabName))
            {
                p.prefabType = PrefabObject.PrefabType.Enemy;
                enemyPrefabs.Add(p);
                Enemies.Add(p.prefabName, p);
                Objects.Add(p.prefabName, p);
            }
            else if (effectNames.Contains(p.prefabName))
            {
                p.prefabType = PrefabObject.PrefabType.Effect;
                effectPrefabs.Add(p);
                Effects.Add(p.prefabName, p);
                Objects.Add(p.prefabName, p);
            }
            else if (hazardNames.Contains(p.prefabName))
            {
                p.prefabType = PrefabObject.PrefabType.Hazard;
                hazardPrefabs.Add(p);
                Hazards.Add(p.prefabName, p);
                Objects.Add(p.prefabName, p);
            }

            Dev.Log("running setup prefab");

            //TODO:get proper type here
            var config = GetPrefabConfig(p, typeof(DefaultPrefabConfig));
            if (config != null)
            {
                config.SetupPrefab(p);
            }
        }
        ...
        IPrefabConfig GetPrefabConfig(PrefabObject p, Type defaultType = null)
        {
            string typeName = "EnemyRandomizerMod." + string.Join("",p.prefabName.Split(' ')) + "PrefabConfig";
            ...
            else
            {
                ...
                {
                    Dev.LogError($"configType given is not an ISpawner: {configType}");
                    return null;
                }
            }
            ...
            if (configType == null && defaultType != null)
            {
                ...
                {
                    Dev.LogError($"Default configType given is not an ISpawner: {defaultType}");
                    return null;
                }
        ...
        ISpawner GetSpawner(PrefabObject p, Type defaultType = null)
        {
            ...
            {
                Dev.Log("Trying to spawn default type");
            ...
            if (spawnerType == null)
            {
                Dev.Log("Type to spawn is null");
                return null;
            }

            Dev.Log("Type to spawn is " + spawnerType.Name);
